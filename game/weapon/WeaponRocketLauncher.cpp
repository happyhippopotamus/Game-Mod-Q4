...
void rvWeaponRocketLauncher::Spawn(void) {
	...
	spawnArgs.GetFloat("lockRange", "100", guideRange);

	spawnArgs.GetFloat("lockSlowdown", ".25", f);
	attackDict.GetFloat("speed", "5000.0f", guideSpeedFast);
	guideSpeedSlow = guideSpeedFast * f;

	reloadRate = SEC2MS(spawnArgs.GetFloat("reloadRate", "10"));
	...
	if (animNum) {
		...
		rate = (float)anim->Length() / (float)SEC2MS(spawnArgs.GetFloat("reloadRate", "10"));
